...
from get_bbh_results import main as eval_main
...
def load_bbh_data(data_dir="data/bbh", max_examples_per_task=None):
    ...
    if not paths:
        raise FileNotFoundError(f"No BBH files found in {data_dir}/bbh_*_test.jsonl")
    ...
    for path in paths:
        ...
        with open(path) as fin:
            for line in fin:
                ...
                test_data.append({
                    "id": example["id"],
                    "dataset": example.get("dataset", "bbh"),
                    "task": example["task"],
...
def main(args):
    ...
    print("Loading BBH data...")
    test_data = load_bbh_data(
        data_dir=args.bbh_data_dir,
        max_examples_per_task=args.max_examples_per_task,
    )
    print(f"Loaded {len(test_data)} BBH examples.")
    ...
    prefix="Answer the following questions. You should think step-by-step. Put only the final answer within \\boxed{}.\n"
    prompts = []
    ...
    model = LLM(model=args.model_name_or_path, tokenizer=args.tokenizer_name_or_path if args.tokenizer_name_or_path else args.model_name_or_path, swap_space=16, gpu_memory_utilization=0.90, enable_lora=args.peft is not None, tensor_parallel_size=torch.cuda.device_count(), max_lora_rank=128, max_model_len=args.max_tokens+2000)
