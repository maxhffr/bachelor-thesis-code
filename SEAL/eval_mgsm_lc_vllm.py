...
def main(args):
    ...
    if args.dataset == "GSM8K":
        data_path = "data/gsm8k/gsm8k_test.jsonl"
        with open(data_path) as fin:
            for line in fin:
                example = json.loads(line)
                test_data.append({
                    "question": example["question"],
                    "answer": "",
                    "gt": str(example["gt"]).strip(),
                })
    elif args.dataset == "MGSM":
        LANGS = [args.lang] if args.lang else ["de", "fr", "es", "ru", "zh", "ja"]
        for lang in LANGS:
            data_path = f'data/mgsm/mgsm_{lang}_test.jsonl'
            with open(data_path) as fin:
                for line in fin:
                    example = json.loads(line)
                    test_data.append({
    ...
    PREFIX_BY_LANG = {
        "de": "Beantworte die folgenden Fragen. Du solltest Schritt für Schritt denken und deine finale Antwort in \\boxed{} setzen.\n",
        "fr": "Réponds aux questions suivantes. Tu dois réfléchir étape par étape et mettre ta réponse finale dans \\boxed{}.\n",
        "es": "Responde las siguientes preguntas. Debes pensar paso a paso y poner tu respuesta final dentro de \\boxed{}.\n",
        "ru": "Ответь на следующие вопросы. Ты должен рассуждать шаг за шагом и поместить окончательный ответ в \\boxed{}.\n",
        "zh": "回答以下问题。你应该一步一步地思考，并将最终答案放在 \\boxed{} 中。\n",
        "ja": "次の質問に答えてください。段階的に考え、最終的な答えを \\boxed{} の中に入れてください。\n",
    }

    DEFAULT_PREFIX = "Answer the following questions. You should think step-by-step and put your final answer within \\boxed{}.\n"


    prompts = []
    for i, example in enumerate(test_data):
        if args.dataset == "MGSM":
            lang = example.get("language", args.lang)
            prefix = PREFIX_BY_LANG.get(lang, DEFAULT_PREFIX)
        else:
