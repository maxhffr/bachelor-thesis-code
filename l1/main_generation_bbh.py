...
# This file is used to evaluate bbh with a different evaluation pipeline to make sure the results get extracted correctly
...
def main(config):
    ...
    if os.path.exists(config.data.output_path):
        ...
        for batch_idx in range(num_batch):
            ...
            print(
                ...
                f"[{batch_idx+1}/{num_batch}] Start to process. "
                f"batch_chat_size={len(batch_chat_lst)}",
                flush=True
            )
            ...
            print(len(data.batch['input_ids']), flush=True)
    ...
    for i in range(total):
        ...
        num_tokens = reward_data.get("num_tokens", -1)
        #extra_info = reward_data.get("extra_info", {})
        #task = extra_info.get("task", "")
        
        num_tokens = reward_data.get("num_tokens", -1)

        task = ""
        if "extra_info" in dataset.columns and isinstance(dataset["extra_info"][i], dict):
            task = dataset["extra_info"][i].get("task", "")

        if not task and isinstance(reward_data, dict):
            task = reward_data.get("extra_info", {}).get("task", "")
        score_lst = []
        for j,r in enumerate(response_lst):
            valid_response_length = dataset["token_lengths"][i][j]
            # if config.rollout.ignore_think_token:
            score = reward_fn(r, ground_truth, num_tokens=num_tokens, valid_response_length=valid_response_length, task=task)
            # else:
...
def select_reward_fn(data_source):
    ...
    elif data_source == 'bbh':
        from math_reward import bbh_reward_fn
        return bbh_reward_fn
    else:
