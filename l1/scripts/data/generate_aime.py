...
for num_tokens in [512, 1024, 2048, 3600, -512, -1024, -2048, -3600, -1]:
    ...
    if num_tokens == -1:
        pd.DataFrame(all_data).to_parquet(f'{out_dir}/data/aime2025.parquet')
    else:
        if num_tokens < 0:
            pd.DataFrame(all_data).to_parquet(f'{out_dir}/data9_{num_tokens}/aime2025.parquet')
        else:
            pd.DataFrame(all_data).to_parquet(f'{out_dir}/data_{num_tokens}/aime2025.parquet')
