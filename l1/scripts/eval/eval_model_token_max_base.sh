...
DATATYPES=("aime" "math" "amc" "minerva" "olympiad_bench" "gsm8k_1000")
...
for DATA_TYPE in "${DATATYPES[@]}"; do
    python3 main_generation.py \
        ...
        data.path=$WORK/l1/artifacts/deepscaler_${NUM_TOKENS}/${DATA_TYPE}.parquet \
        data.output_path=${OUTPUT_DIR}_max_base_${NUM_TOKENS}/${DATA_TYPE}.parquet \
        data.n_samples=16 \
        data.batch_size=64 \
